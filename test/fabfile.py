        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.test.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.test.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.test.proxy_django.include",
                "filename": "app.test.proxy_django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.test.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "default_site": True,
...
def test():
    ...
    defaults()
    env.hosts = ['test.test.rhizophore.com']
...
def deploy(id=None, silent=False, force=False, auto_nginx=True):
    ...
    app_patterns = [r' test/app', r' test/static',
                    r' test/settings', r' test/package.json']
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/test')
...
def setup_server(id=None):
    ...
    # Create database
    sudo('echo "CREATE DATABASE test; '
         '      CREATE USER test WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE test to test;" '
         '| docker exec -i postgres-10 psql -U postgres'.format(db_password=db_password))

    # Upload local settings
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/test/settings/local.py', use_sudo=True)
    ...
    sudo('touch %s/test/app/webpack-stats.json' % env.code_dir)
    ...
    # Copy logrotate conf
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/test')
...
def collectstatic(npm_build=True):
    if npm_build:
        docker_compose_run('node', 'npm run build', name='test_npm_build')
...
def docker_compose_run(service, cmd='', name='test_tmp'):
...
def ensure_docker_networks():
    ...
    ensure_docker_network_exists('test_default', [], internal=False)
    ensure_docker_network_exists('test_nginx', ['nginx'])
    ensure_docker_network_exists('test_postgres', ['postgres-10'])
